...
type resolve_module_result =
  (Cpath.Resolved.module_ * Component.Module.t Component.Delayed.t, Cpath.module_) ResolvedMonad.t
 ...
 fun ~mark_substituted ~add_canonical env' path ->
  ...
  let resolve env (mark_substituted, add_canonical, p) =
    ...
    | `Identifier(i, hidden) as unresolved ->
        ...
        >>= fun (`Module (_, m)) ->
        let p = if hidden then `Identifier i else `Hidden (`Identifier i) in
        return (process_module_path env ~add_canonical
                  (Component.Delayed.get m) p, m)
...
and resolve_type : Env.t -> Cpath.type_ -> resolve_type_result =
  ...
  fun env p ->
    ...
    | `Dot (parent, id) as unresolved ->
        ...
        return (p', t)
    | `Identifier(i, _) as unresolved ->
        of_result ~unresolved (lookup_type env (`Identifier i))
