...
let module_signature_cache :
    (Component.Signature.t * int * Env.lookup_type list) Memos3.t =
  Memos3.create 10000
...
and process_module_type env m p' =
  ...
  try
    ...
    p'
  with
  | OpaqueModule -> `OpaqueModuleType p'
  | UnresolvedForwardPath -> p'
...
and handle_module_lookup env add_canonical id parent sg =
  ...
  | Replaced p -> Some (p, lookup_module env p)
  | exception _e -> None
...
and handle_class_type_lookup env id p m =
  let sg =
    prefix_signature (`Module p, signature_of_module_cached env p true m)
  in
    ...
    | `Module (parent, name) -> (
        let sg =
          ...
          | `Module mp ->
              ...
              prefix_signature
                (parent, signature_of_module_cached env mp false parent_module)
          | `ModuleType mtyp ->
              let parent_module_type = lookup_module_type env mtyp in
              prefix_signature
                (parent, signature_of_module_type env parent_module_type)
          | `FragmentRoot ->
...
and lookup_module_type :
    Env.t -> Cpath.Resolved.module_type -> Component.ModuleType.t =
 fun env path ->
  let lookup env =
    match path with
 ...
 fun is_resolve add_canonical env' p ->
  ...
  let resolve : Env.t -> (module_lookup_result, Cpath.module_) ResultMonad.t =
   fun env ->
    ...
    | `Dot (parent, id) -> (
        ...
        let sg =
          prefix_signature
            (`Module p, signature_of_module_cached env p is_resolve m)
        in
  ...
  | [] ->
      ...
      let lookups, resolved =
        try Env.with_recorded_lookups env' resolve
        with e ->
      ...
      let rec find = function
        | Component.Signature.Module (id, _, m') :: _
          when Ident.Name.module_ id = name ->
            ( id,
              `Module (`Module ppath, ModuleName.of_string name),
              Component.Delayed.get m' )
        | _ :: xs -> find xs
        | [] -> failwith "Can't find it"
      in
...
and module_type_expr_of_module_decl :
    Env.t -> Component.Module.decl -> Component.ModuleType.expr =
 fun env decl ->
  match decl with
...
and module_type_expr_of_module :
    Env.t -> Component.Module.t -> Component.ModuleType.expr =
 fun env m -> module_type_expr_of_module_decl env m.type_

and signature_of_module_alias : Env.t -> Cpath.module_ -> Component.Signature.t
    =
 fun env path ->
  match lookup_and_resolve_module_from_path false true env path with
  | Resolved (p', m) ->
...
and signature_of_module : Env.t -> Component.Module.t -> Component.Signature.t =
  ...
  | `Dot (parent, name) ->
      ...
      let f'' =
        try
          ...
          f'
        with
        | OpaqueModule -> `OpaqueModule f'
        | UnresolvedForwardPath -> f'
      in
...
let rec class_signature_of_class :
    Env.t -> Component.Class.t -> Component.ClassSignature.t =
 fun env c ->
  let rec inner decl =
    match decl with
...
and class_signature_of_class_type_expr :
    Env.t -> Component.ClassType.expr -> Component.ClassSignature.t =
 fun env e ->
  match e with
  | Signature s -> s
  | Constr (p, _) -> (
...
and class_signature_of_class_type :
    Env.t -> Component.ClassType.t -> Component.ClassSignature.t =
